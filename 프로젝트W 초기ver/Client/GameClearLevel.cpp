...
{
    m_tex = AssetMgr::GetInst()->LoadTexture(L"GameClear", L"Texture\\GameClear_1440x1080.bmp");
}
...
void GameClearLevel::Enter()
{
	m_bgm = AssetMgr::GetInst()->LoadSound(L"GameClearBGM", L"Sound\\GameClearBGM.wav");
	m_bgm->PlayToBGM(true);
}

void GameClearLevel::Tick()
{
    // 게임 재시작
...
void GameClearLevel::Exit()
{
}

void GameClearLevel::Render(HDC _dc)
{
    if (m_tex)
    {
    ...
    // 상단 Game Over 텍스트
    const wchar_t* gameOverText = L"ESCAPE SUCCESS";
    ...
    // === 하단 안내문구 ===
    const wchar_t* retryText = L"다시 해보기 : Enter";
